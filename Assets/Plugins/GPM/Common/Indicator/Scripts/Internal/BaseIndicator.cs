...
namespace Gpm.Common.Indicator.Internal
{
    public class BaseIndicator
    {
        ...
        protected virtual void GetConfig(Action<Config> callback)
        {
            ...
            coroutineObject.StartCoroutine(
                helper.SendWebRequestAndDispose(result =>
                {
        ...
        protected virtual void GetLaunchingInfo(Config config, Action<LaunchingInfo> callback)
        {
            ...
            coroutineObject.StartCoroutine(
                helper.SendWebRequestAndDispose(result =>
                {
                    ...
                    {
                        var launchingInfo = GpmJsonMapper.ToObject<LaunchingInfo>(
        ...
        {
            queue.Enqueue(
                new QueueItem(serviceName, serviceVersion, body, customData, ignoreActivation)
            );
        }
        ...
        {
            coroutineObject.StartCoroutine(
                helper.SendWebRequestAndDispose(
                    (result) =>
                    {
        ...
        {
            return (
                (result == null) || (string.IsNullOrEmpty(result.downloadHandler.text) == true)
            );
        }
