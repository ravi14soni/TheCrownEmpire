...
namespace Gpm.Common.Compress.Internal
{
    ...
    {
#if UNITY_EDITOR_OSX
        private static readonly string COMPRESS_PROGRAM_PATH = Path.Combine(
            Path.GetDirectoryName(EditorApplication.applicationContentsPath),
            "Contents/Tools/7za"
        );
        ...
        private static Dictionary<CompressFormat, string> formatExtensions = new Dictionary<
            CompressFormat,
            string
        >
        {
        ...
        internal static CompressResultCode ExtractCode(string filePath, string outputPath)
        {
            ...
            catch (Exception e)
            {
                UnityEngine.Debug.LogError(
                    string.Format("Extract process error (Message: {0})", e.Message)
                );
        ...
        internal static CompressResultCode Extract(
            string filePath,
            string outputPath,
            ...
            using (var process = Process.Start(startInfo))
            {
                try
                {
                    ...
                    if (exitCode != 0)
                    {
                        UnityEngine.Debug.LogError(
                            string.Format(
                                "Extract process error (Arguments: {0}, ExitCode: {1}, Output: {2}, Error: {3})",
                                startInfo.Arguments,
                                exitCode,
                                stdout,
                                stderr
        ...
        private static string GetExtractArgument(
            CompressFormat format,
            string filePath,
            string outputPath
        )
        {
            ...
            if (format == CompressFormat.TAR_GZ)
            {
                ...
                return string.Format(
                    @"-c ""'{0}' x '{1}' -so | '{0}' x -aoa -si -ttar -o'{2}'""",
                    COMPRESS_PROGRAM_PATH,
                    filePath,
                    outputPath
                );
        ...
        {
            public static CompressResultCode Unpack(
                string packagePath,
                string tempPath,
                string resultPath
            )
...
#else
                CompressResultCode extractResultCode = Extract(
                    packagePath,
                    tempFullPath,
            ...
            private static bool RemapPackageToAsset(string originPath, string outputPath)
            {
                ...
                try
                {
                    ...
                    {
                        var remapPath = File.ReadAllLines(
                                GpmPathUtil.Combine(directoryInfo.FullName, "pathname")
                            )
                            .First();
                        ...
                        var metaFilePath = GpmPathUtil.Combine(
                            directoryInfo.FullName,
                            "asset.meta"
                        );

                        GpmFileUtil.MoveFile(
                            assetFilePath,
                            GpmPathUtil.Combine(outputPath, remapPath)
                        );
                        GpmFileUtil.MoveFile(
                            metaFilePath,
                            GpmPathUtil.Combine(outputPath, string.Format("{0}.meta", remapPath))
                        );
                    }
                }
                catch (Exception e)
                {
                    UnityEngine.Debug.LogError(
                        string.Format(
                            "Remap unity package to asset error (Message: {0})",
                            e.Message
                        )
                    );
                    return false;
