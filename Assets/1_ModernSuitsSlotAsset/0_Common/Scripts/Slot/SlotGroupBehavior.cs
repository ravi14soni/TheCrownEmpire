...
namespace Mkey
{
    public class SlotGroupBehavior : MonoBehaviour
    {
        ...
        public void SetReelGrid(List<string> reelGrid)
        {
            for (int i = 0; i < slotSymbols.Length; i++)
            {
                if (i < reelGrid.Count)
                {
                    string symbol = reelGrid[i];
                    //SymbolName
                    /* int symbolIndex = sprites.ToList().FindIndex(s => s.SymbolName == symbol);
                    if (symbolIndex >= 0)
                    {
                        slotSymbols[i].SetIcon(sprites[symbolIndex], symbolIndex);
                    }
                    else
                    {
                        Debug.LogWarning($"Symbol {symbol} not found in sprites.");
                    } */

                    int symbolIndex = sprites.ToList().FindIndex(s => s.Name == symbol);
                    if (symbolIndex >= 0)
                    {
                        slotSymbols[i].SetIcon(sprites[symbolIndex], symbolIndex);
                    }
