...
namespace Mkey
{
    ...
    public class SlotController : MonoBehaviour
    {
        ...
        private float inRotTime = 0.3f;//.3
        [SerializeField]
        [Tooltip("In rotation part angle, 0-10 deg")]
        private float inRotAngle = 7;//5
        ...
        private float outRotTime = 0.3f;//.3
        [SerializeField]
        [Tooltip("Out rotation part angle, 0-10 deg")]
        private float outRotAngle = 7; //7
        ...
        public float mainRotateTime = 5f;
        [Tooltip("min 0% - max 20%, change rotateTime")]
        [SerializeField]
        public int mainRotateTimeRandomize = 10;
        ...
        void Start()
        {
            ...
            CreateFullPaytable();
            //            Debug.Log("reelsData" + reelsData.Length);
        ...
        private void RunSlots()
        {
            ...
            /*             if (!controls.ApplyBet())
                        {
                            MGUI.ShowMessage(null, "You have no money.vivek", 1.5f, null);
                            controls.ResetAutoSpinsMode();
                            return;
                        } */
        ...
        private void RotateSlots(Action rotCallBack)
        {
            ...
            if (controls.UseHold && hold && hold.Length == rands.Length)
            {
                ...
                {
                    rands[i] = slotGroupsBeh[i].CurrOrderPosition;
    ...
    public class RNG
    {
        ...
        {
            Debug.LogError("randSymb.Length" + randSymb.Length);
            for (int i = 0; i < randSymb.Length; i++)
            {
