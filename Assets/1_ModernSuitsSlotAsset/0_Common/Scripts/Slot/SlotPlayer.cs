...
namespace Mkey
{
    public class SlotPlayer : MonoBehaviour
    {
        ...
        public int Coins
        {
            get
            {
                string walletString = PlayerPrefs.GetString("wallet", "0");
                return (int)Math.Round(float.Parse(walletString));
            }
            private set
            {
                PlayerPrefs.SetString("wallet", value.ToString());
                PlayerPrefs.Save();
            }
        }
        public int Level
        ...
        {
            string walletString = PlayerPrefs.GetString("wallet", "0");
            defCoinsCount = (int)Math.Round(float.Parse(walletString));
            Debug.Log("Defalut coins::" + defCoinsCount);
