...
public class TeenPattiSocketManager : MonoBehaviour
{
    ...
    public List<TPTableDataResponse> tablePlayers; // to store seat data
    [Header("Show")]
...
public TextMeshProUGUI ownTimer;
public TextMeshProUGUI pooldetails,comission_set;
    [Header("Add Cash Panel")]
    public GameObject AddCashPanel;
    private bool stopRechargeTimer = false;
    string popupStr = "";
    bool isPrivateTable;
    ...
    private void OnEnable()
    {
        ...
        isPrivateTable = PlayerPrefs.GetInt("SelectedTab") == 0 ? false : true;
        if (isPrivateTable)
        {
            shareObj.SetActive(true);
            roomcode.text = "YOUR ROOM CODE : " + Configuration.GetTableID();
            Debug.Log("ravi private...");
        }
        else
        {
            shareObj.SetActive(false);
            Debug.Log("ravi public...");
        ...
        if (!isPrivateTable)
        {
             walletString = Configuration.GetWallet();
        }
        else
        {
            walletString   = Configuration.GetWinningWallet();
        }
        ...
        if (decimal.TryParse(walletString, out decimal userCoins))
        {
            if (userCoins > 10000)
            {
                ...
                {
                    Wallet_paenl.transform.GetChild(0).GetComponent<TextMeshProUGUI>().text =
                       FormatNumber(Configuration.GetWinningWallet());
                }
        ...
        customNamespace.On<string>("recharge-send", OnRechargeResponse);
    ...
    private void OnTimerResponse(string args)
    {
        ...
        Debug.Log("ChaalTimer" + chaaltimer);
        if (chaaltimer <= 299 && chaaltimer >= 30)
        {
            int minutes = chaaltimer / 60;
            int remainingSeconds = chaaltimer % 60;


            //timerText.text = "Please Wait Player is Recharging " + minutes + ":" + remainingSeconds;
        }
        else if (chaaltimer <= 30)
        {
            ownTimer.text = args;
        }
    ...
    {
        Debug.Log("Slide Show Request "+args);
        slideShowBool = false;

        try {

            SlideShowDoAll slideShowDoAll = JsonUtility.FromJson<SlideShowDoAll>(args);
            Debug.Log("Args slide-show-do..." + slideShowDoAll.yes_slide_show.user_id + "<>" + slideShowDoAll.yes_slide_show.prev_id);

            ArrowShow(slideShowDoAll.yes_slide_show.user_id, slideShowDoAll.yes_slide_show.prev_id);
         }
        catch (System.Exception ex)
        {
            Debug.LogError(ex.ToString());
    ...
    {
        Debug.Log("Slide Show Request Yes /no " + args);
        SlideShowDoAll slideShowDoAll = JsonUtility.FromJson<SlideShowDoAll>(args);
        showtoastmessage(slideShowDoAll.message);
    ...
    private void OnChaalResponse(string args)
    {
        ...
        if (gameresponse.message == "Insufficient Coins")
        {
    ...
    bool isRecharging;
    bool isShow, isSlideShow, ischaal, ischaal2x, isBlind, isBlind2x;

    private void OnRechargeResponse(string args)
    {

        Debug.Log("RES_Value + On_recharge Json :" + args);

        RechargeResponseAll responseData = JsonUtility.FromJson<RechargeResponseAll>(args);
        string rechargetext = "Player is Recharging";


        int index = GetProfileIndex(responseData.recharge_data[0].user_id);
        Debug.Log("Recharge response index: " + index + "currendid" + profiles[index].GetComponent<ChaalSlider>().id);

        // check if same player
        if (responseData.recharge_data[0].action == "4")
        {
            string currentId = profiles[index].GetComponent<ChaalSlider>().id;
            if (Configuration.GetId() == currentId)
            {
                AddCashPanel.SetActive(true);
                // ⚙️ Start coroutine for delay
                StartCoroutine(RechargeTimerLocal());

                isRecharging = true;

                isShow = show.activeInHierarchy;
                isSlideShow = slideshow.activeInHierarchy;
                isBlind = blindbtn.activeInHierarchy;
                isBlind2x = blind2x.activeInHierarchy;
                ischaal = chaalbtn.activeInHierarchy;
                ischaal2x = chaal2x.activeInHierarchy;

                show.SetActive(false);
                slideshow.SetActive(false);
                blindbtn.SetActive(false);
                blind2x.SetActive(false);
            ...
            if (index >= 0 && index < profiles.Count)
            {
                ...
                // choose child index based on profile index
                int childIndex = (index > 0) ? 9 : 11;

                Transform parent = profiles[index].transform.GetChild(0);

                TMP_Text rechargeTextObj = parent.GetChild(childIndex).GetChild(0)
                    .GetComponent<TMP_Text>();

                rechargeTextObj.text = rechargetext;

                parent.GetChild(childIndex).gameObject.SetActive(true);


                Debug.Log($"Recharge text set on profile {index}, child {childIndex}");
            }
            else
            {
                Debug.Log($"⚠️ Invalid profile index: {index}");
            }
            ...
            for(int i=0;i<profiles.Count;i++)
            {
                profiles[i].GetComponent<ChaalSlider>().isRechargingUser = true;
            }

        }
    }

    public void OnRechargeSuccessResponse()
    {
        try
        {
            show.SetActive(isShow);
            slideshow.SetActive(isSlideShow);
            blindbtn.SetActive(isBlind);
            blind2x.SetActive(isBlind2x);
            chaalbtn.SetActive(ischaal);
            chaal2x.SetActive(ischaal2x);
    ...
    IEnumerator RechargeWait()
    {
        ...
        showtoastmessage("Recharge is successfull!");
    }

    // 👇 Make a separate coroutine for the delay
    private IEnumerator HideRechargePanelAfterDelay()
    {
    ...
    private void OnStartGameResponse(string args)
    {
        ...
        {
            Debug.Log("Game ID" + gameresponse.game_id + "args" + args);
 ...
 private void OnGetTableResponse(string args)
{
    ...
    try
    {
        ...
        tablePlayers = tableDataList;           // store server seats
        TableID = tableDataList[0].table_id;
        tableID.text = "Table ID: " + TableID;
        roomcode.text = "YOUR ROOM CODE : " + TableID;

        // ✅ Find your own seat
        foreach (var p in tablePlayers)
        {
            if (p.user_id == Configuration.GetId())
            {
                mySeat = int.Parse(p.seat_position);
                Debug.Log("✅ My Seat Position: " + mySeat);
            }
        }

        // ✅ Assign seat number to every ChaalSlider
       foreach (var tp in tablePlayers)
{
    foreach (GameObject profile in profiles)
    {
        var cs = profile.GetComponent<ChaalSlider>();
        if (cs != null && cs.id == tp.user_id)
        {
            cs.seatNumber = int.Parse(tp.seat_position);
            Debug.Log($"Assigned Seat {cs.seatNumber} to {cs.id}");
        }
    }
}

// Reorder visuals so profile indexes are in clockwise order (optional)



        // ✅ Re-order profiles clockwise for visual order
        //ReorderProfilesBySeat();

        comission_set.text =
            "2. Charging " + response.comission +
            "% commission on the winning pot as a platform fee.";
...
public void ReorderProfilesBySeat() //ravi analysis...
{
    // Sort profiles by seatNumber ascending (1 => bottom, 2 => right, 3 => top, 4 => left)
    profiles = profiles
        .OrderBy(p => {
            var cs = p.GetComponent<ChaalSlider>();
            return cs != null ? cs.seatNumber : int.MaxValue;
        })
        .ToList();

    Debug.Log("Profiles reordered by seatNumber:");
    foreach (var p in profiles)
    {
        var cs = p.GetComponent<ChaalSlider>();
        if (cs != null)
            Debug.Log($"Profile id:{cs.id} seat:{cs.seatNumber}");
    }

    // Visual placement - adjust positions to match your UI
    foreach (var p in profiles)
    {
        var cs = p.GetComponent<ChaalSlider>();
        if (cs == null) continue;
        switch (cs.seatNumber)
        {
            case 1: p.transform.localPosition = new Vector3(0, -300, 0); break; // bottom
            case 2: p.transform.localPosition = new Vector3(350, 0, 0); break;  // right
            case 3: p.transform.localPosition = new Vector3(0, 300, 0); break;  // top
            case 4: p.transform.localPosition = new Vector3(-350, 0, 0); break; // left
            default: break;
        }
    }
}
 ...
 int GetProfileIndex(string id)
    ...
    {
        Debug.Log("RES_Check + On_trigger" + args);
        ...
        {
            string usid = PlayerPrefs.GetString("id");
            gameID = args;
            Debug.Log("Game ID : " + args + " user id " + usid);
            API_CALL_status();
            // if (!string.IsNullOrEmpty(gameID))
            // {
            //     CheckRecharge();
    ...
    public IEnumerator closemoveanim(GameObject player, int move)
    {
        ...
        {
            Debug.Log("Action Canva "+ profiles[i].GetComponent<ChaalSlider>().ischaal);
            if (profiles[0].GetComponent<ChaalSlider>().id != TPresponseData.game_users[i].user_id)
            {
                Debug.Log("Action "+TPresponseData.game_log[0].action);
                //Debug.Log("Bot Action User id"+ TPresponseData.game_log[i].user_id + " " +profiles[0].GetComponent<ChaalSlider>().id);
                ...
                if (TPresponseData.game_log[0].action == "1")
                {
                    popupStr = "Packed";
                }
            }
        ...
        {
            Debug.Log("Chaal Current "+ player.GetComponent<ChaalSlider>().id + " Current " +Configuration.GetId() + "Amount " +amount);
        ...
        {
            Debug.Log("Else Chaal Current "+ player.GetComponent<ChaalSlider>().id + " Current " +Configuration.GetId() + "Amount " +amount);
    ...
    private void SpawnBetCoin(string userId, string amountStr, bool addToTotal = true)
    {
        ...
        Debug.Log("Coin ravi spawn:" + userId);
...
{
    if (TPresponseData == null || TPresponseData.game_log == null || TPresponseData.game_log.Count == 0)
        return;

    // Use the last log entry (most recent move)
    var log = TPresponseData.game_log.Last();
    Debug.Log("RES_Check + amount " + log.amount + " user:" + log.user_id);

    // Only act when move changed
    if (moveid != log.user_id)
    {
        moveid = log.user_id;
        Debug.Log("Coin ravi chaal " + log.user_id);

        if (!isRecharging)
        {
            // Spawn coin animation
            SpawnBetCoin(log.user_id, log.amount, true);
        }
        ...
        if (showmove)
        {
            ...
            // Find seat from server tablePlayers
            
            foreach (var tp in tablePlayers)
            {
                if (tp.user_id == log.user_id)
                {
                    if (int.TryParse(tp.seat_position, out mySeat)) break;
                }
            }

            // Find player GameObject by seatNumber (preferred)
            if (mySeat != -1)
            {
                player = profiles.FirstOrDefault(
                    p => {
                        var cs = p.GetComponent<ChaalSlider>();
                        return cs != null && cs.seatNumber == mySeat;
                    });
            }

            // Fallback: find by id if seat not found
            if (player == null)
            {
                player = profiles.FirstOrDefault(
                    p => {
                        var cs = p.GetComponent<ChaalSlider>();
                        return cs != null && cs.id == moveid;
                    });
            }
            ...
            Debug.Log($"Seat Position: {moveid} seatNum:{mySeat}");
        }
    }
}
 ...
 void ArrowShow(string currentuserid, string previousid)
    {
        StartCoroutine(ArrowShowWait(currentuserid, previousid));
    }


     IEnumerator ArrowShowWait(string currentUserID, string previousUserID)
    {
        yield return new WaitForSeconds(0.5f);
        ...
        for (int i = 0; i < profiles.Count; i++)
        {
            if (profiles[i].GetComponent<ChaalSlider>().id == currentUserID)
            {
                startPos = profiles[i].transform.GetChild(0).transform.position;
            }
            if (profiles[i].GetComponent<ChaalSlider>().id == previousUserID)
            {
                targetPos = profiles[i].transform.GetChild(0).transform.position;
            }
        }
    ...
    IEnumerator Request()
    {
        ...
        using (UnityWebRequest request = UnityWebRequest.Post(url, form))
        {
            ...
            else
            {
                ...
                int _gameid = int.Parse(gameID);
                if (_gameid > 0)
                {
                    CheckRecharge(gameID);
                }
                WalletMoney();

                showTableUsers();

                // NEW: Integration - Check for other players recharging (if it's their turn and they lack coins)
                string currentChaalId = TPresponseData.chaal;
                float requiredBet = float.Parse(TPresponseData.table_amount);
                if (currentChaalId != Configuration.GetId())
                {
                    string playerName = "";
                    for (int i = 0; i < TPresponseData.all_users.Count; i++)
                    {
                        if (TPresponseData.all_users[i].user_id == currentChaalId)
                        {
                            playerName = TPresponseData.all_users[i].name;
                            break;
                        }
                    }
                ...
                pooldetails.text = "1. Maximum Blind = " + TPresponseData.table_detail.maximum_blind + ", Pot Limit = " + TPresponseData.table_detail.pot_limit + ", Boot Value = " + TPresponseData.table_detail.boot_value;
                ...
                if (TPresponseData.game_log.Count != 0 && startinganim)
                {
                    ...
                    Debug.Log("Coin ravi chaal uper" + TPresponseData.game_log.Count + "<startinganim>" + startinganim + "<TPresponseData.game_status>" + TPresponseData.game_status);
                    ChaalAnim();
                }
                ...
                if (TPresponseData.game_status == 1 && !startinganim)
                {
                    ...
                    Debug.Log("Coin ravi status" + TPresponseData.game_log.Count + "<startinganim>" + startinganim + "<TPresponseData.game_status>" + TPresponseData.game_status);
                ...
                if (!gameover)
                {
                            ...
                            )
                            {
                                if (i == 0)
                                    ...
                                    else
                                    {
                                        ...
                                        .GetComponent<TextMeshProUGUI>()
                                        .text =
                                        FormatNumber(TPresponseData.all_users[j].winning_wallet) + "";
                                    }
                    ...
                    //User Amount = float.Parse(TPresponseData.game_log[0].amount);
    ...
    private void WalletMoney()
    {
        ...
        //check for current has enough coin in wallet to play
        for (int i = 0; i < TPresponseData.all_users.Count; i++)
        {
            if (profiles[0].GetComponent<ChaalSlider>().id == TPresponseData.all_users[i].user_id)
            {
                Debug.Log("ravi.." + TPresponseData.all_users[i].wallet + "<table_amount>" + TPresponseData.table_amount);
                float _wallet;
                if (!isPrivateTable)
                    _wallet = float.Parse(TPresponseData.all_users[i].wallet);
                else
                    _wallet = float.Parse(TPresponseData.all_users[i].winning_wallet);
                float tableAmount = float.Parse(TPresponseData.table_amount);
                
                if (_wallet < tableAmount)
                {
                    isRecharging = true;
     ...
     public void slideshowno()
    {
        ...
        try
        {
            ...
            Debug.Log(TPresponseData.slide_show[0].user_id + " slide ID for Silde yes Emit");
            Debug.Log(jsonData.type + " type for Silde yes Emit");
            Debug.Log("RES_VALUE" + "Slide Show request" + jsonData);
        }
        catch (System.Exception e)
        {
            Debug.LogError(e.ToString());
    ...
    public void GameSideShow()
    {
        ...
        try
        {
            ...
            // arrowPreID = previousid;
            //arrowCurrID = profiles[0].GetComponent<ChaalSlider>().id;
    ...
    private void CheckRecharge(string gameid)
    {
        var customNamespace = Manager.GetSocket(name_space);
            ...
            var data = new Dictionary<string, object>()
        {
            ...
            // Emit to socket
            customNamespace.Emit("recharge-send", jsonStr);
    ...
    public void showTableUsers()
    {
        ...
        List<TPUserData> arrangeList = new List<TPUserData>();
        for (int i = 0; i < placementdata.Count; i++)
        {
            if (i <= index)
            {
                arrangeList.Add(placementdata[index - i]);
            }
            else
            {
                arrangeList.Add(placementdata[i]);
            }
        }

        placementdata.Clear();
        placementdata = arrangeList;

        // Move the item to the first position if it's not already in the first position
        // if (index > 0)
        // {
        //     TPUserData userData = placementdata[index];//ravi analysis...
        ...
        for (int i = 0; i < placementdata.Count; i++)
        {
            ...
            else
            {
                ...
                if (i == 0)
                    ...
                    {
                        Wallet_paenl.transform.GetChild(0).GetComponent<TextMeshProUGUI>().text =
                       FormatNumber(placementdata[i].winning_wallet);
                    }
                ...
                else
                {
                    profiles[i]
                        ...
                        .GetChild(0)
                        .GetComponent<TextMeshProUGUI>()
                        .text = FormatNumber(placementdata[i].winning_wallet);
                }
    ...
    private IEnumerator RechargeTimerLocal()
    {
        float timer = 300f;
        TextMeshProUGUI timerText = AddCashPanel.GetComponentInChildren<TextMeshProUGUI>();
        if (timerText == null)
        {
            Debug.LogError("Timer TextMeshProUGUI not found in AddCashPanel!");
            yield break;
        }
        while (timer > 0 && !stopRechargeTimer)
        {
            //timerText.text = "Please add cash to continue... (" + Mathf.CeilToInt(timer) + "s)";
...
[Serializable]
public class RechargeResponseAll
{
    public string message;
    public List<RechargeResponse> recharge_data;
}
[Serializable]
...
[Serializable]
public class SlideShowDoAll
{
    public SlideShowDo yes_slide_show;
    public string message;
}
