...
public class PoolPositioner : MonoBehaviour
{
    public RectTransform panel;
    public List<GameObject> cards;
    public float panelWidth;
    public Vector3 panelPosition;
    public GameManager_Pool obj;
    public bool active;
    ...
    void Start()
    {
        obj = GameObject.Find("GameManager").GetComponent<GameManager_Pool>();
        if (!obj.gamestart)
        {
    ...
    void SetPanelPosition()
    {
        if (!obj.isDragging)
        {
            ...
            if (cards.Count == 1)
            {
                ...
                panel.anchoredPosition = new Vector2(cards[0].transform.position.x, panel.anchoredPosition.y);
