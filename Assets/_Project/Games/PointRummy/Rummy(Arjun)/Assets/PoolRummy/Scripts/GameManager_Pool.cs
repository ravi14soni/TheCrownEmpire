...
public class GameManager_Pool : MonoBehaviour
{
    #region variables
    [Header("Iniialization")]
    public PoolRummyConnection poolrummymanager;
    ...
    public int numoflist,
        numofmatch,
        numofmatchcards;
    public int numberofpuresequence,
    ...
    public GameObject discradedcardfromtrigger;
    ...
    IEnumerator rummycardsspawn()
    {
        ...
        names = poolrummymanager.cardNames;
    ...
    )
    {
        ...
        if (list1.Count != 0)
        {
            textcavas[0].GetComponent<PoolPositioner>().cards = list1;
            textcavas[0].SetActive(true);
        }
        if (list2.Count != 0)
        {
            textcavas[1].GetComponent<PoolPositioner>().cards = list2;
            textcavas[1].SetActive(true);
        }
        if (list3.Count != 0)
        {
            textcavas[2].GetComponent<PoolPositioner>().cards = list3;
            textcavas[2].SetActive(true);
        }
        if (list4.Count != 0)
        {
            textcavas[3].GetComponent<PoolPositioner>().cards = list4;
            textcavas[3].SetActive(true);
        }
        if (list5.Count != 0)
        {
            textcavas[4].GetComponent<PoolPositioner>().cards = list5;
            textcavas[4].SetActive(true);
        }
        if (list6.Count != 0)
        {
            textcavas[5].GetComponent<PoolPositioner>().cards = list6;
    ...
    #region RummyCards to be spawned and noved to desired locations
    ...
    {
        int value = 0;
        wildcardname = poolrummymanager.gameDataList[0].joker;
    ...
    //rearranging all the cards according to their suits
    ...
    void HandleInput()
    {
        ...
        )
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                {
                    getcard = false;
                    if (ischaal && !poolrummymanager.gameDataList[0].declare)
                    {
                        if (spawnedCards.Count == 13)
                        {
                            StartCoroutine(clicked());

                            poolrummymanager.API_CALL_get_card();
                ...
                if ((clickedCard.name == "Discard_Area" && getcard))
                {
                    ...
                    if (ischaal && !poolrummymanager.gameDataList[0].declare)
                    {
                        if (spawnedCards.Count == 13)
                        {
                            ...
                            {
                                poolrummymanager.API_CALL_get_drop_card();
    ...
    //when cards come back to the list
                ...
                if (Physics.Raycast(ray, out hit))
                {
                    ...
                    else if (releasedCard.name == "FinishDesk")
                    {
                        StartCoroutine(
                            showtext(
                                warning,
                                "You have 13 cards, please draw a card from deck and then Finish the game."
                            )
                        );
                    }
                    ...
                    {
                        Debug.Log("RES_Check + ReleasedCard + " + releasedCard);
    ...
    //when card is discarded from server and we get the response
    public void DiscardAfterONResponse(GameObject card, string area)
    {
        ...
        if (isdeclared)
        {
            Debug.Log("RES_Check + Declare on drop response");
            if (numofmatchcards == 13 || numofmatchcards == 14)
            {
                Debug.Log("RES_Check + Declare numofmatchcards " + numofmatchcards);
                if (isvaliddeclare())
                    poolrummymanager.API_CALL_declare();
                else
                {
                    //poolrummymanager.API_CALL_Wrongdeclare();
                    isdeclared = false;
                    finished = false;
                    Destroy(finishdeskcard);
                    finishdeskcard = null;
                    Debug.Log("RES_Check + Wrong Declare");
                    card.transform.position = GameObject.Find("Discard_Area").transform.position;
                    showtoastmessage("Cannot accept wrong declare");
                }
            }
            else
            {
    ...
    public void AutoDiscardCard(GameObject card)
    {
        ...
        if (card != null)
        {
            ...
            else
            {
                ...
                {
                    poolrummymanager.Idropped = droppedcard;
        ...
        //        poolrummymanager.Idropped = droppedcard;
        //        Debug.Log("RES_Check + " + droppedcard + " DiscardCard");
        //        GameObject.Find("ProfilePic").transform.GetChild(0).GetComponent<PoolRummyChaalSlider>().discardglow.SetActive(false);
        //        GameObject.Find("ProfilePic").transform.GetChild(0).GetComponent<PoolRummyChaalSlider>().finishdeskglow.SetActive(false);
        //    }
        //    else
        //    {
        //        Debug.LogError("Card name is null or empty.");
    ...
    public void DiscardCard(GameObject card)
    {
        ...
        if (card != null)
        {
            ...
            if (!string.IsNullOrEmpty(droppedcard))
            {
                poolrummymanager.Idropped = droppedcard;
                Debug.Log("RES_Check + " + droppedcard + " DiscardCard");
                poolrummymanager.API_CALL_drop_card(droppedcard);
                GameObject
                    .Find("ProfilePic")
                    .transform.GetChild(0)
                    .GetComponent<PoolRummyChaalSlider>()
    ...
    IEnumerator waitfordropcardresponse()
    {
        ...
        //showtoastmessage("Facing connectivity issues");
        if (dropcardresponse)
        {
            Destroy(discardCard);
            dropcardresponse = false;
    ...
    void ShiftLists()
    {
        ...
        else if (list3.Count == 0 && list2.Count != 0)
        {
            ShiftElements(list4, list3);
            ShiftElements(list5, list4);
            ShiftElements(list6, list5);
        }
        else if (list4.Count == 0 && list3.Count != 0)
        {
            ShiftElements(list5, list4);
            ShiftElements(list6, list5);
        }
        else if (list5.Count == 0 && list4.Count != 0)
        {
            ShiftElements(list6, list5);
        }
    }
    ...
    //when we click on yes after we declare the game
    public void Finishyes()
    {
        if (numofmatchcards == 13 || numofmatchcards == 14)
        {
            //finishdeskcard.GetComponent<Card>().name.ToLower() != "jkr1" && finishdeskcard.GetComponent<Card>().name.ToLower() != "jkr2"
            if (
            ...
            )
            {
                ...
                canvastext(list1, list2, list3, list4, list5, list6);
                CheckAfterEveryMove();
                poolrummymanager.API_CALL_drop_card(
                    finishdeskcard.GetComponent<Card>().name.ToUpper()
                );
                finished = false;
                declaredialogue.SetActive(false);
            }
        ...
        else
        {
            StartCoroutine(
                showtext(
                    warning,
                    "Wrong Declare, Please declare only after all the groups are valid"
                )
            );
            finished = false;
            finishdeskcard.transform.localScale = new Vector3(1.5f, 1.5f, 1.5f);
    ...
    //when we get the card from the deck
    public IEnumerator InatntiateSingleCard(string objectplaceholder)
    {
        yield return new WaitForSeconds(0.0f);

        string newaskedcard = poolrummymanager.askedCard;
    ...
    //getting card from discarded area
    ...
    public void CheckAfterEveryMove()
    {
        ...
        {
            seqcheck++;
            textcavas[0].GetComponent<PoolPositioner>().active = true;
        }
        if (check2)
        {
            seqcheck++;
            textcavas[1].GetComponent<PoolPositioner>().active = true;
        }
        if (check3)
        {
            seqcheck++;
            textcavas[2].GetComponent<PoolPositioner>().active = true;
        }
        if (check4)
        {
            seqcheck++;
            textcavas[3].GetComponent<PoolPositioner>().active = true;
        }
        if (check5)
        {
            seqcheck++;
            textcavas[4].GetComponent<PoolPositioner>().active = true;
        }
        if (check6)
        {
            seqcheck++;
            textcavas[5].GetComponent<PoolPositioner>().active = true;
        }
        ...
        if (check1)
            ...
            textcavas[0].GetComponent<PoolPositioner>().active = false;
        if (check2)
            textcavas[1].GetComponent<PoolPositioner>().active = true;
        else
            textcavas[1].GetComponent<PoolPositioner>().active = false;
        if (check3)
            textcavas[2].GetComponent<PoolPositioner>().active = true;
        else
            textcavas[2].GetComponent<PoolPositioner>().active = false;
        if (check4)
            textcavas[3].GetComponent<PoolPositioner>().active = true;
        else
            textcavas[3].GetComponent<PoolPositioner>().active = false;
        if (check5)
            textcavas[4].GetComponent<PoolPositioner>().active = true;
        else
            textcavas[4].GetComponent<PoolPositioner>().active = false;
        if (check6)
            textcavas[5].GetComponent<PoolPositioner>().active = true;
        else
            textcavas[5].GetComponent<PoolPositioner>().active = false;
        ...
        for (int i = 0; i < puresequenceint; i++)
        {
            if (
                textcavas[puresequenceint]
                    ...
                    .text != "Pure Sequence (0)"
            )
                puresequence = false;

            if (
                textcavas[sequencelist].transform.GetChild(0).GetComponent<TextMeshProUGUI>().text
                != "Impure Sequence (0)"
            )
                sequence = false;
        }
    ...
    public void showinvalidnum(GameObject canvas, List<GameObject> list)
    {
        ...
        canvas.GetComponent<PoolPositioner>().active = false;
    }
