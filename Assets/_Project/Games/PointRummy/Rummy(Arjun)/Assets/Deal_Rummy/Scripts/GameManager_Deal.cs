...
public class GameManager_Deal : MonoBehaviour
{
    #region variables
    [Header("Iniialization")]
    public DRummyConnection dealrummymanager;
    ...
    IEnumerator rummycardsspawn()
    {
        ...
        names = dealrummymanager.cardNames;
    ...
    )
    {
        ...
        if (list1.Count != 0)
        {
            textcavas[0].GetComponent<Deal_positioner>().cards = list1;
            textcavas[0].SetActive(true);
        }
        if (list2.Count != 0)
        {
            textcavas[1].GetComponent<Deal_positioner>().cards = list2;
            textcavas[1].SetActive(true);
        }
        if (list3.Count != 0)
        {
            textcavas[2].GetComponent<Deal_positioner>().cards = list3;
            textcavas[2].SetActive(true);
        }
        if (list4.Count != 0)
        {
            textcavas[3].GetComponent<Deal_positioner>().cards = list4;
            textcavas[3].SetActive(true);
        }
        if (list5.Count != 0)
        {
            textcavas[4].GetComponent<Deal_positioner>().cards = list5;
            textcavas[4].SetActive(true);
        }
        if (list6.Count != 0)
        {
            textcavas[5].GetComponent<Deal_positioner>().cards = list6;
    ...
    {
        int value = 0;
        wildcardname = dealrummymanager.gameDataList[0].joker;
    ...
    void HandleInput()
    {
        ...
        )
        {
            ...
            if (Physics.Raycast(ray, out hit))
            {
                ...
                {
                    Debug.Log("RES_check + Clicked Placeholder before");
                    getcard = false;
                    if (ischaal && !dealrummymanager.gameDataList[0].declare)
                    {
                        Debug.Log("RES_check + Clicked Placeholder");
                        if (spawnedCards.Count == 13)
                        {
                            StartCoroutine(clicked());

                            dealrummymanager.API_CALL_get_card();
                ...
                {
                    Debug.Log("RES_check + Clicked discard before");
                    Debug.Log("RES_Value + " + clickedCard.name);
                    getcard = false;
                    if (ischaal && !dealrummymanager.gameDataList[0].declare)
                    {
                        Debug.Log("RES_check + Clicked discard");
                        if (spawnedCards.Count == 13)
                        {
                            int wildrank = 0;
                            ...
                            {
                                dealrummymanager.API_CALL_get_drop_card();
    ...
    public void AutoDiscardCard(GameObject card)
    {
        ...
        if (card != null)
        {
            ...
            else
            {
                ...
                {
                    dealrummymanager.Idropped = droppedcard;
        ...
        //            dealrummymanager.Idropped = droppedcard;
        //            Debug.Log("RES_Check + " + droppedcard + " DiscardCard");
        //            GameObject.Find("ProfilePic").transform.GetChild(0).GetComponent<DealRummyChaalSlider>().discardglow.SetActive(false);
        //            GameObject.Find("ProfilePic").transform.GetChild(0).GetComponent<DealRummyChaalSlider>().finishdeskglow.SetActive(false);
        //        }
        //        else
        //        {
        //            Debug.LogError("Card name is null or empty.");
        ...
        if (isdeclared)
        {
            if (numofmatchcards == 13 || numofmatchcards == 14)
            {
                if (isvaliddeclare())
                {
                    canvastext(list1, list2, list3, list4, list5, list6);
                    CheckAfterEveryMove();
                    dealrummymanager.API_CALL_declare();
                }
                else
                {
                    //dealrummymanager.API_CALL_Wrongdeclare();
                    isdeclared = false;
                    finished = false;
    ...
    public void DiscardCard(GameObject card)
    {
        ...
        if (card != null)
        {
            ...
            if (!string.IsNullOrEmpty(droppedcard))
            {
                dealrummymanager.Idropped = droppedcard;
                Debug.Log("RES_Check + " + droppedcard + " DiscardCard");
                dealrummymanager.API_CALL_drop_card(droppedcard);
                GameObject
                    .Find("ProfilePic")
                    .transform.GetChild(0)
                    .GetComponent<DealRummyChaalSlider>()
    ...
    public void rearrangenewgroup(GameObject card)
    {
        ...
        //AddObjectAfterNewCard(card, lists[emptylistindex]);
    ...
    public void Finishyes()
    {
        if (numofmatchcards == 13 || numofmatchcards == 14)
        {
            ...
            )
            {
                ...
                canvastext(list1, list2, list3, list4, list5, list6);
                CheckAfterEveryMove();
                dealrummymanager.API_CALL_drop_card(
        ...
        else
        {
            StartCoroutine(
                showtext(
                    warning,
                    "Wrong Declare, Please declare ony after all the groups are valid"
                )
            );
            finished = false;
            finishdeskcard.transform.localScale = new Vector3(1.5f, 1.5f, 1.5f);
    ...
    public IEnumerator InatntiateSingleCard(string objectplaceholder)
    {
        ...
        string newaskedcard = dealrummymanager.askedCard;
    ...
    public void CheckAfterEveryMove()
    {
        ...
        {
            seqcheck++;
            textcavas[0].GetComponent<Deal_positioner>().active = true;
        }
        if (check2)
        {
            seqcheck++;
            textcavas[1].GetComponent<Deal_positioner>().active = true;
        }
        if (check3)
        {
            seqcheck++;
            textcavas[2].GetComponent<Deal_positioner>().active = true;
        }
        if (check4)
        {
            seqcheck++;
            textcavas[3].GetComponent<Deal_positioner>().active = true;
        }
        if (check5)
        {
            seqcheck++;
            textcavas[4].GetComponent<Deal_positioner>().active = true;
        }
        if (check6)
        {
            seqcheck++;
            textcavas[5].GetComponent<Deal_positioner>().active = true;
        }
        ...
        if (check1)
            ...
            textcavas[0].GetComponent<Deal_positioner>().active = false;
        if (check2)
            textcavas[1].GetComponent<Deal_positioner>().active = true;
        else
            textcavas[1].GetComponent<Deal_positioner>().active = false;
        if (check3)
            textcavas[2].GetComponent<Deal_positioner>().active = true;
        else
            textcavas[2].GetComponent<Deal_positioner>().active = false;
        if (check4)
            textcavas[3].GetComponent<Deal_positioner>().active = true;
        else
            textcavas[3].GetComponent<Deal_positioner>().active = false;
        if (check5)
            textcavas[4].GetComponent<Deal_positioner>().active = true;
        else
            textcavas[4].GetComponent<Deal_positioner>().active = false;
        if (check6)
            textcavas[5].GetComponent<Deal_positioner>().active = true;
        else
            textcavas[5].GetComponent<Deal_positioner>().active = false;
    ...
    public void showinvalidnum(GameObject canvas, List<GameObject> list)
    {
        ...
        canvas.GetComponent<Deal_positioner>().active = false;
    }
