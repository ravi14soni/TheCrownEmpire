...
public class Deal_positioner : MonoBehaviour
{
    public RectTransform panel;
    public List<GameObject> cards;
    public float panelWidth;
    public Vector3 panelPosition;
    public GameManager_Deal obj;
    public bool active;
    public Sprite red,
        green;
    ...
    void Start()
    {
        obj = GameObject.Find("GameManager").GetComponent<GameManager_Deal>();
        if (!obj.gamestart)
        {
    ...
    void SetPanelPosition()
    {
        if (!obj.isDragging)
        {
            ...
            if (cards.Count == 1)
            {
                ...
                panel.position = new Vector3(panel.position.x, -2.1f, panel.position.z);
