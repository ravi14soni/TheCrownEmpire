...
namespace Mkey
{
    public class SceneLoader : MonoBehaviour
    {
        ...
        private AsyncOperationHandle<SceneInstance> asyncDynamicLoad;
        ...
        private GuiController MGUI
        {
            ...
            {
                if (!mGUI)
                    mGUI = FindObjectOfType<GuiController>();
                return mGUI;
            }
        }
        ...
        public void LoadScene(string sceneName)
        {
            Debug.Log(
                "Scene name "
                    + sceneName
                    + " index "
                    + SceneManager.GetSceneByName(sceneName).buildIndex
            );
            //int scene = SceneManager.GetSceneByName(sceneName).buildIndex;
            StartCoroutine(AsyncLoadBeatyName(sceneName, null, null));
        }
        ...
        public void LoadDynamicScene(string sceneName)
        {
            ...
            //int scene = SceneManager.GetSceneByName(sceneName).buildIndex;
            StartCoroutine(AsyncDynamicLoadBeatyName(sceneName, null, null));
        }
        ...
        private IEnumerator AsyncLoadBeaty(
            int scene,
            Action<float> progresUpdate,
        ...
        )
        {
            ...
            if (LoadGroup)
                simpleSlider = LoadGroup.GetComponentInChildren<PSlider>();
            if (simpleSlider)
                simpleSlider.SetFillAmount(loadProgress);
        ...
        private IEnumerator AsyncLoadBeatyName(
            string scene,
            Action<float> progresUpdate,
            Action completeCallBack
        )
        ...
        private IEnumerator AsyncDynamicLoadBeatyName(
            string scene,
            Action<float> progresUpdate,
            Action completeCallBack
        )
        {
            ...
            asyncDynamicLoad = Addressables.LoadSceneAsync(scene);
            //asyncDynamicLoad.allowSceneActivation = false;
            float lastTime = Time.time;

            while (loadProgress < 0.99f || asyncDynamicLoad.PercentComplete < 0.90f)
            {
                loadTime += (Time.time - lastTime);
                lastTime = Time.time;
                loadProgress = Mathf.Clamp01(loadProgress + iStep);
                ...
                if (loadTime >= 0.5f * apprLoadTime && (asyncDynamicLoad.PercentComplete < 0.5f))
                {
                    apprLoadTime *= 1.1f;
                    apprLoadTime = Mathf.Min(apprLoadTimeMax, apprLoadTime);
                }
                else if (
                    loadTime >= 0.5f * apprLoadTime
                    && (asyncDynamicLoad.PercentComplete > 0.5f)
                )
                {
                    apprLoadTime /= 1.1f;
                    apprLoadTime = Mathf.Max(apprLoadTimeMin, apprLoadTime);
